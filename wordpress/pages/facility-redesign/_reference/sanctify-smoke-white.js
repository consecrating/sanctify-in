(function(){"use strict";if(window.innerWidth<=1000){return}
if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion:reduce)').matches){return}
function initSmoke(canvas){try{canvas.width=canvas.clientWidth;canvas.height=canvas.clientHeight;let config={SIM_RESOLUTION:128,DYE_RESOLUTION:512,DENSITY_DISSIPATION: 0.93,VELOCITY_DISSIPATION:0.98,PRESSURE_DISSIPATION:0.8,PRESSURE_ITERATIONS:20,CURL:30,SPLAT_RADIUS:0.1,SHADING:!1,COLORFUL:!1,PAUSED:!1,BACK_COLOR:{r:0,g:0,b:0},TRANSPARENT:!1,BLOOM:!1,BLOOM_ITERATIONS:8,BLOOM_RESOLUTION:256,BLOOM_INTENSITY:0.8,BLOOM_THRESHOLD:0.6,BLOOM_SOFT_KNEE:0.7};function pointerPrototype(){this.id=-1;this.x=0;this.y=0;this.dx=0;this.dy=0;this.down=!1;this.moved=!1;this.color=[30,0,300]}
...
initFramebuffers();multipleSplats(parseInt(Math.random()*20)+5);let lastColorChangeTime=Date.now();update();setInterval(function(){try{if(config.PAUSED)return;var x=canvas.width*(0.08+0.84*Math.random());var y=canvas.height*(0.55+0.5*Math.random());var dx=(Math.random()-0.5)*130;var dy=-(Math.random()*210+70);var color=generateColor();color.r*=2.4;color.g*=2.4;color.b*=2.4;splat(x,y,dx,dy,color);}catch(e){}},760);function update(){resizeCanvas();input();if(!config.PAUSED)step(0.016);render(null);requestAnimationFrame(update)}
...
document.body.addEventListener("mousemove",(e)=>{pointers[0].moved=pointers[0].down;var _r=canvas.getBoundingClientRect();var _cx=e.clientX-_r.left,_cy=e.clientY-_r.top;pointers[0].dx=(_cx-pointers[0].x)*5.0;pointers[0].dy=(_cy-pointers[0].y)*5.0;pointers[0].x=_cx;pointers[0].y=_cy});document.body.addEventListener("mousemove",()=>{pointers[0].down=!0;pointers[0].color=generateColor()});function generateColor(){var pal=[[0.62,0.62,0.70],[0.55,0.55,0.64],[0.70,0.70,0.78],[0.58,0.60,0.68]];var c=pal[Math.floor(Math.random()*pal.length)];return{r:c[0],g:c[1],b:c[2]}}
...
function getTextureScale(texture,width,height){return{x:width/texture.width,y:height/texture.height}}}catch(e){}}['sf-smoke-white'].forEach(function(id){var c=document.getElementById(id);if(c){initSmoke(c)}})})()