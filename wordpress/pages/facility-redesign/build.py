...
/* ---- reveal: pure on-load CSS animation, ALWAYS ends visible (no JS / observer dependency) ---- */
@keyframes sfy-in{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:none}}
.sfy2 .r{animation:sfy-in .7s cubic-bezier(.2,.7,.3,1) both}
@media (prefers-reduced-motion:reduce){
  .sfy2 .r{animation:none}
...
<script>
(function(){
  var root=document.querySelector('.sfy2');
  if(!root||root.dataset.sfyInit)return; root.dataset.sfyInit='1';
  // count-up (default DOM text already shows the real value, so this only enhances)
