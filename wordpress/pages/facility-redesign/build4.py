...
os.makedirs("preview4", exist_ok=True)
...
def render(pid, d):
    ...
    parts.append(services_section(*d["grid_head"], grid(d["grid"])))
...
for pid, d in PAGES.items():
    ...
    open(f"build4/{d['slug']}.content.html","w").write(content)
    json.dump(payload, open(f"build4/{d['slug']}.payload.json","w"), ensure_ascii=False)
    ...
    open(f"preview4/{d['slug']}.html","w").write(prev)
