...
class AutoKeywordLinks {
    ...
    private function split_html_tags($html) {
        ...
        $regex = '/(<[^>]+>)/i';
        $pieces = preg_split($regex, $html, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
        $out = array();
        foreach ($pieces as $piece) {
            if (preg_match($regex, $piece)) {
                $out[] = array('tag' => true, 'content' => $piece);
            } else {
                $out[] = array('tag' => false, 'content' => $piece);
            }
        }
        ...
        foreach ($out as $p) {
            if ($p['tag']) {
                if ($buffer !== '') {
                    $merged[] = array('tag' => false, 'content' => $buffer);
                    $buffer = '';
                }
                $merged[] = $p;
            } else {
        ...
        // Further split text by protected tags so we don't replace inside them
        $final = array();
        foreach ($merged as $m) {
            if ($m['tag']) {
                $final[] = $m;
            } else {
                $sub = preg_split('/(<(a|script|style|code|pre)\b[^>]*>.*?<\/\2>)/is', $m['content'], -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
                foreach ($sub as $s) {
                    if (preg_match('/^<(a|script|style|code|pre)\b/i', $s)) {
                        $final[] = array('tag' => true, 'content' => $s);
                    } else {
                        $final[] = array('tag' => false, 'content' => $s);
                    }
                }
            }
        }
