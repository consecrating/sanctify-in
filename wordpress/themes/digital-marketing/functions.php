...
if($qode_options_sanctify['show_toolbar'] == "yes"):
...
function myStartSession() {
    if(!session_id()) {
		...
		if (!empty($_GET['top_menu']))
			$_SESSION['qode_top_menu'] = $_GET['top_menu'];
...
function qode_styles() {
	...
	$fonts_array  = array(
						$qode_options_sanctify['google_fonts'].':200,300,400',
						$qode_options_sanctify['menu_google_fonts'].':200,300,400',
						$qode_options_sanctify['dropdown_google_fonts'].':200,300,400',
						$qode_options_sanctify['dropdown_google_fonts_thirdlvl'].':200,300,400',
						$qode_options_sanctify['h1_google_fonts'].':200,300,400',
						$qode_options_sanctify['h2_google_fonts'].':200,300,400',
						$qode_options_sanctify['h3_google_fonts'].':200,300,400',
						$qode_options_sanctify['h4_google_fonts'].':200,300,400',
						$qode_options_sanctify['h5_google_fonts'].':200,300,400',
						$qode_options_sanctify['h6_google_fonts'].':200,300,400',
						$qode_options_sanctify['text_google_fonts'].':200,300,400',
						$qode_options_sanctify['slider_title_google_fonts'].':200,300,400',
						$qode_options_sanctify['slider_title_google_fonts_type2'].':200,300,400',
						$qode_options_sanctify['slider_text_google_fonts'].':200,300,400',
						$qode_options_sanctify['page_title_google_fonts'].':200,300,400',
						$qode_options_sanctify['button_title_google_fonts'].':200,300,400',
						$qode_options_sanctify['message_title_google_fonts'].':200,300,400'
						);
...
function qode_scripts() {
	...
	if (($qode_options_sanctify['page_transitions'] != "0") && (empty($qode_animation) || ($qode_animation != "no")))
		$has_ajax = true;
	...
	if($qode_options_sanctify['use_recaptcha'] == "yes") :
...
add_image_size( 'blog-type-1-small', 233, 233, true );
add_image_size( 'blog-type-1-medium', 263, 263, true );
add_image_size( 'blog-type-1-big', 360, 360, true );
add_image_size( 'blog-type-3-small', 196, 111, true );
add_image_size( 'blog-type-3-medium', 223, 127, true );
add_image_size( 'blog-type-3-big', 313, 177, true );
add_image_size( 'blog-type-4-small', 308, 174, true );
add_image_size( 'blog-type-4-medium', 350, 198, true );
add_image_size( 'blog-type-4-big', 485, 361, true );
add_image_size( 'blog-type-5-big', 460, 260, true );
add_image_size( 'latest_posts', 320, 173, true );
}
...
function ajax_classes($classes) {
	...
	if(($qode_options_sanctify['page_transitions'] === "0") && ($qode_animation == "no")) :
		$classes[] = '';
	elseif($qode_options_sanctify['page_transitions'] === "1" && (empty($qode_animation) || ($qode_animation != "no"))) :
		$classes[] = 'ajax_updown';
		$classes[] = 'page_not_loaded';
	elseif($qode_options_sanctify['page_transitions'] === "2" && (empty($qode_animation) || ($qode_animation != "no"))) :
		$classes[] = 'ajax_fade';
		$classes[] = 'page_not_loaded';
	elseif($qode_options_sanctify['page_transitions'] === "3" && (empty($qode_animation) || ($qode_animation != "no"))) :
		$classes[] = 'ajax_updown_fade';
		$classes[] = 'page_not_loaded';
	elseif($qode_options_sanctify['page_transitions'] === "4" && (empty($qode_animation) || ($qode_animation != "no"))) :
...
function boxed_class($classes) {
	...
	if($qode_options_sanctify['boxed'] == "yes") :
		$classes[] = 'boxed';
	else:
	$classes[] ="";
	endif;
...
function qode_excerpt_length( $length ) {
	global $qode_options_sanctify;
	if($qode_options_sanctify['number_of_chars']){
		 return $qode_options_sanctify['number_of_chars'];
	} else {
		return 45;
	}
}
...
function my_theme_register_required_plugins() {
	 ...
	 */
	$config = array(
    'id'           => $theme_text_domain,       // Unique ID for hashing notices
    'default_path' => '',                       // Default absolute path to bundled plugins
    'menu'         => 'install-required-plugins', // Menu slug
    'parent_slug'  => 'themes.php',             // ✅ replace parent_menu_slug
    'capability'   => 'edit_theme_options',     // Recommended capability
    'has_notices'  => true,                     // Show admin notices or not
    'dismissable'  => true,                     // User can dismiss the notice
    'is_automatic' => false,                    // Auto activate after install?
    'message'      => '',                       // Message output before plugins table
    'strings'      => array(
        ...
        'nag_type'                        => 'updated', // Only 'updated' or 'error'
    ),
);
